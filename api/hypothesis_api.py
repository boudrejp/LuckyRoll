...
def all_dice_calculator():
    ...
    for die in dice:
        ...
        die_sides = np.array(list(range(1, die['sides'] + 1)))
        rolls = np.random.multinomial(len(die['rolls']), [1/die['sides']] * die['sides'], size = n_simulations) * die_sides
        ...
        actuals_rolls_counts = count_each_roll(die['sides'], die['rolls'])
